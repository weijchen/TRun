Shader "Custom/moveLight 1" {
	Properties {
		...
		//_NewLightWidth("NewLightWidth", Range(1, 20)) = 1
	}
	SubShader {
		...
		float4 _AnotherColor;
		float _SpeedY;
		float _SpeedX;
		float _LightWidth;
		//float _NewLightWidth;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			//float2 uv1 = IN.uv_MainTex / _NewLightWidth;
			//流光图y轴偏移
			//uv1.y += _Time.y * _SpeedY + 0.5;
			//流光图x轴偏移
			//uv1.x += _Time.y * _SpeedX;
			//获取流光图的蓝色通道，因为流光图都是黑白，获取红色绿色通道都可以
			fixed light = tex2D(_LightTex, uv).b;
			//fixed light1 = tex2D(_LightTex, uv1).b;
